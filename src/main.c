...
static void Draw_GameUI(SDL_Renderer *renderer, GameState state)
{
    ...
    const char *title = state == GAME_MENU ? "DOG ALIVE" :
                        state == GAME_PAUSED ? "PAUSED" : "GAME OVER";
    ...
    if (state == GAME_MENU) {
        SDL_RenderDebugTextFormat(renderer, 24, 200, "Ghoul:%d Slime:%d Eye:%d",
            GHOUL_COUNT, FLESH_SLIME_COUNT, EYE_PARASITE_COUNT);
        SDL_RenderDebugText(renderer, 48, 144, "WASD / Arrows: Move");
        SDL_RenderDebugText(renderer, 48, 160, "Space: Slash  J: Shoot");
        SDL_RenderDebugText(renderer, 48, 176, "Esc: Pause");
    } else {
        SDL_RenderDebugText(renderer, 104, 136, "M: Main menu");
    }
}

static void Reset_Game(Player *p, EnemyGroup *enemies,
                       Projectile *arrows, SlimeShot *shots, RunningEffect *effect)
{
    Game_Init(p);
    EnemyGroup_Init(enemies);
...
static SDL_Texture *CreateArrowTexture(SDL_Renderer *renderer)
{
    ...
    for (int y = 0; y < surface->h; ++y) {
        ...
        for (int x = 0; x < surface->w; ++x) {
            uint16_t color = arrow_projectile_frame_1[y * surface->w + x];
            Uint8 *pixel = row + x * 4;
...
static void Draw_Projectiles(SDL_Renderer *renderer, SDL_Texture *texture,
...
int main(void)
{
    ...
    SDL_Texture *arrowTexture = renderer ? CreateArrowTexture(renderer) : NULL;
    if (!arrowTexture) {
    ...
    Reset_Game(&player, &enemies, projectiles, slimeShots, &runningEffect);
    ...
    while (running)
    {
        ...
        if (gameState == GAME_PLAYING) {
            ...
            if (player.hp > 0)
            {
                ...
                player.shootTimer = fmaxf(0.0f, player.shootTimer - deltaTime);
        ...
        SDL_SetWindowTitle(window, gameState == GAME_MENU ? "Dog Alive | Enter: Start" :
            ...
            "WASD/Arrows: move/aim | Space: slash | J: shoot | Esc: pause | R: restart");
