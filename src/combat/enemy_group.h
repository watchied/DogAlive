...
{
    *group = (EnemyGroup){.ghoulCount = GHOUL_COUNT,
        .slimeCount = FLESH_SLIME_COUNT, .eyeCount = EYE_PARASITE_COUNT};
    int slot = 0;
                                     ...
                                     float step, float tip, float radius, int damage)
{
    ...
    EnemyGroup_Damage(g, targets[nearest], damage, ux * (10.0f / 3.0f), uy * (10.0f / 3.0f));
    return true;
}
static inline void EnemyGroup_Arrows(EnemyGroup *g, Projectile *shots, float dt)
{
    for (int i = 0; i < MAX_PROJECTILES; ++i) {
        ...
        if (EnemyGroup_Sweep(g, s->x, s->y, s->vx, s->vy, step, 5.5f, 0, s->damage)) {
